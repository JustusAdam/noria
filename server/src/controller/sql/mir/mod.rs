...
fn vec_replace_with_where<T: Clone, F: Fn(&T) -> bool>(v: &mut Vec<T>, f: F, t: &T) {
    for i in 0..v.len() {
        if f(&v[i]) {
            v[i] = t.clone()
        }
...
impl SqlToMirConverter {
    ...
    > {
        ...
        Ok((
            sec,
            self.expand_graph_udfs(MirQuery {
                name: String::from(name),
                roots,
                leaf,
            }),
    ...
    fn make_udf_node(
        ...
        MirNode::new(
            ...
            MirNodeType::UDF {
                ...
                group_by: if group_cols.is_empty() {
                    Option::None
                } else {
                    Option::Some(group_cols.iter().map(|c| (*c).clone()).collect())
                },
            },
            vec![parent.clone()],
            vec![],
    ...
    fn make_function_node(
        ...
        match *func.deref() {
            UDF(ref fun, ref cols) => {
                out_nodes.push(self.make_udf_node(name, func_col, group_cols, parent, fun, cols));
                return out_nodes;
            }
            _ => {}
        }
    ...
    fn make_grouped_node(
        ...
        match node_type {
            ...
            GroupedNodeType::FilterAggregation(filter_agg) => mknode({
                use nom_sql::ConditionExpression::*;
    ...
    // Add initial project
    pub fn expand_graph_udfs(&self, query: MirQuery) -> MirQuery {
        for node_ref in query.iter() {
            ...
            match node.inner {
                MirNodeType::UDF {
                    ...
                    if udfs::UDTFIncorporator::new(self.log.clone()).is_defined(function_name) {
                        ...
                        node.inner = MirNodeType::UDFBasic {
                            ...
                            indices: input.clone(),
                            execution_type: if let Some(g) = group_by {
                                ExecutionType::Reduction{group_by: g.clone()}
                            } else {
                                ExecutionType::Simple{carry:input.len()}
                            }
                        };
