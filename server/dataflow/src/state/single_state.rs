...
impl Leaf for Vec<Row> {
    ...
    fn remove(&mut self, r: &[DataType]) -> Option<Row> {
        if self.len() == 1 {
            ...
            let left = self.drain().next().unwrap();
...
impl<T: Leaf> SingleState<T> {
    ...
    where T: Default
    {
        ...
        let replaced = match self.state {
            KeyedState::Single(ref mut map) => map.insert(key.next().unwrap(), T::default()),
            KeyedState::Double(ref mut map) => {
                map.insert((key.next().unwrap(), key.next().unwrap()), T::default())
            }
